...
export default async(app, storeManager) => {
    if ((process.env.VUE_APP_DOCHUB_CLUSTER || 'off').toLowerCase() === 'on') {
        ...
        logger.log('info', `Hash of cluster updated to [${app.storage?.hash}]`, LOG_TAG);
        ...
        const clusterMiddleware = async function(req, res, next) {
            ...
            if (remoteHash !== app.storage?.hash) {
                logger.log('warn', `Cluster inconsistency detected. Current hash is [${app.storage?.hash}], remote hash is [${remoteHash}]. Reloading manifest...`, LOG_TAG);
                storeManager.cleanStorage(app);
                storeManager.reloadManifest(app)
                .then(async(storage) => {
                    await storeManager.applyManifest(app, storage);
                    logger.log('info', `Reloading complete. Current hash is [${app.storage.hash}], remote hash is [${remoteHash}].`, LOG_TAG);
                })
                .catch((err) => {
